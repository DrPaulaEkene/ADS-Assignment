...
"""
# Assignment Instruction 1 Starts, the line plots
...
class DataFrame:
    """
    A class encapsulating my Pandas DataFrame with additional methods.

    Attributes:
        df (DataFrame): The Pandas DataFrame object.
    """

    def __init__(self, data):
        """
        Initialize my CustomDataFrame object.

        Parameters:
            data (dict): A dictionary or data structure to create my DataFrame.
        """
...
def delete_rows(dataframe, condition):
    """
    Deletes rows from the dataframe based on a specified condition.

    Parameters:
    - dataframe: the DataFrame from which rows are to be deleted.
    - condition: boolean Series/expression that specifies the rows to delete.
...
# Remove '<'to convert all Results to integers,fix non-numeric & missing values
df_campylo_d.loc[:, "Result"] = pd.to_numeric(
    df_campylo_d["Result"].str.replace("<", ""),
...
plt.plot(x, z, label="Sample Collection Temperature", color="black")
...
# Assignment Instruction 2 Starts,(2a, the Piecharts)
...
CType_T.columns = ['Value', '%Contribution per Chicken type']
print(CType_T)

# Plot and save a pie chart, representing the percentage of each type
explode = (0.1, 0, 0.5, 0.1)
CType_T.plot(kind='pie', y='%Contribution per Chicken type',
             labels=CType_T['Value'], autopct='%1.1f%%',
             startangle=110, legend=False, explode=explode)
plt.axis('equal')
plt.savefig("Chickentypes%.png")
plt.show()

# Assignment Instruction 2 Continues,(2b, the barcharts)
...
plt.savefig("Chickensources.png")
plt.show()
